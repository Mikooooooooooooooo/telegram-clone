<template> 
    <div class="login">
        <div class="container">
            <telegram-logo class="logo"></telegram-logo> 
...
export default {
    ...
    methods: {
        ...
        async toAuth() {
            try {
                const res = await api.post('/check-user' , {phone: this.number }) 
                if (res.data.exists) {
                    this.$router.push({name: 'Login' , query: {phone: this.number}})
                } else {
                    this.$router.push({name: 'Register' , query: {phone: this.number}})
                }
            } catch (err) {
